...
Origin: https://github.com/elldeeone/agent-scripts (forked from steipete/agent-scripts).

This folder collects guardrail helpers so they are easy to reuse in other repos or share during onboarding.
...
- Keep every file dependency-free and portable: the scripts must run in isolation across repos. Do not add `tsconfig` path aliases, shared source folders, or any other repo-specific imports—inline tiny helpers or duplicate the minimum code needed so the mirror stays self-contained.
...
## Sync Expectations
- This repository is the canonical mirror for the guardrail helpers used across your projects. Whenever you edit `scripts/committer`, `scripts/docs-list.ts`, or related guardrail files in another repo, copy the changes back here immediately (and vice versa) so the code stays byte-identical.
...
## @elldeeone Agent Instructions (pointer workflow)
